...
if ( ! function_exists( 'jaguza_optionsframework_mlu_init' ) ) {
	function jaguza_optionsframework_mlu_init () {
		register_post_type( 'options', array(
			'labels' => array(
				'name' => __( 'Options','jaguza' ),
			),
			'public' => true,
			'show_ui' => false,
...
add_filter( 'get_media_item_args', 'jaguza_force_send' );
function jaguza_force_send($args){
	$args['send'] = true;
	return $args;
}
...
if ( ! function_exists( 'jaguza_optionsframework_mlu_css' ) ) {

	function jaguza_optionsframework_mlu_css () {
...
if ( ! function_exists( 'jaguza_optionsframework_mlu_js' ) ) {

	function jaguza_optionsframework_mlu_js () {
	
		// Registers custom scripts for the Media Library AJAX uploader.
		wp_register_script( 'of-medialibrary-uploader', Jaguza_ADMIN_DIR .'assets/js/of-medialibrary-uploader.js', array( 'jquery', 'thickbox' ) );
 ...
 *
 * Example Usage:
 * jaguza_optionsframework_mlu_get_silentpost ( 'of_logo' );
 */

if ( ! function_exists( 'jaguza_optionsframework_mlu_get_silentpost' ) ) {

	function jaguza_optionsframework_mlu_get_silentpost ( $_token ) {
...
if ( ! function_exists( 'jaguza_optionsframework_mlu_insidepopup' ) ) {

	function jaguza_optionsframework_mlu_insidepopup () {
	
		if ( isset( $_REQUEST['is_optionsframework'] ) && $_REQUEST['is_optionsframework'] == 'yes' ) {
		
			add_action( 'admin_head', 'jaguza_optionsframework_mlu_js_popup' );
			add_filter( 'media_upload_tabs', 'jaguza_optionsframework_mlu_modify_tabs' );
		}
	}
}

if ( ! function_exists( 'jaguza_optionsframework_mlu_js_popup' ) ) {

	function jaguza_optionsframework_mlu_js_popup () {
...
if ( ! function_exists( 'jaguza_optionsframework_mlu_modify_tabs' ) ) {

	function jaguza_optionsframework_mlu_modify_tabs ( $tabs ) {
