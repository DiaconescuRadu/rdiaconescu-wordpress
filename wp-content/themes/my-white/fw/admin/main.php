...
class mytheme_admin {
    ...
    static function pageHeader( $pageSlug )
    {
        ...
        echo '<span class="theme"><strong>' . myTheme::name() . '</strong> ' . __( 'Version' , 'myThemes' ) . ': ' . myTheme::version() . '</span>';
        echo '<a href="http://mythem.es" target="_blank" title="Affordable WordPress Themes For Your Website or Blog"><img src="' . DEV_LOGO . '" /></a>';
        ...
        if( !empty( $_POST ) ){
            foreach( $_POST as $key => & $d ){
                if( substr( $key , 0 , 9 ) == 'mythemes-' ){
                    ...
                    /* deb::e( $key . ' ' . $_POST[ $key ] . ' ' . ahtml::getValidator( $st[ $fName ] ) ); */
