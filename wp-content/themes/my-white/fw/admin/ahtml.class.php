...
class ahtml {
    ...
    function _uploadID( $sett )
    {
        ...
        $result .= self::getButtonClass( $sett , true , 'button-upload_' ) . ' ';
        $result .= ' value="' . __( 'Choose File' , "myThemes" ) . '" ';
        $result .= self::_getUploadIDAction( self::getInputID( $sett ) ) . ' />';
    ...
    function upload( $sett )
    {
        ...
        $result .= ' value="' . __( 'Choose File' , "myThemes" ) . '" ';
        $result .= ' onclick="javascript:mytheme_fl_upload( jQuery( this ).parent().children(\'input#' . self::getInputID( $sett ) . '\') , \'#my-uploader-box\' )"/>';
        
        if( isset( $sett[ 'size' ] ) )
            my_fl_upload::run( $sett[ 'size' ] );
        else
            my_fl_upload::run( );
    ...
    function myThemesDocs( $url ){
            $result  = '<iframe src="' . $url . '" ';
            $result .= 'width="100%" height="700" scrolling="yes" frameborder="0"></iframe>';
